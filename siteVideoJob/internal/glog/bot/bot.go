...
var cacheKey = "SITE_BOT_WARNING_CACHE_%s_%s"
...
func SendDefaultWithTimeout(template, position, msg string, timeout int64) {
	SendWithTimeout("初始站点ID", template, position, msg, timeout)
}
func SendWithTimeout(siteId, template, position, msg string, timeout int64) {
	...
	//相同的代码位置，timeout期间内只推送一次，防止大批量推送导致系统奔溃和漏掉通知
	//如果不存在就从redis去获取,如果redis存在，然后刷到内存，并直接返回
	if !ok {
		rs, err := core.GetKey(key)
		if rs != "" && err == nil {
	...
	if !ok {
		...
		core.SetExpireKV(key, strconv.FormatInt(now, 10), time.Second*time.Duration(timeout))
	}
}

// 推送用的预设间隔时间
func Send(siteId, template, position, msg string) {
	SendWithTimeout(siteId, template, position, msg, timeout)
}
func SendDefault(template, position, msg string) {
	SendWithTimeout("初始化站点", template, position, msg, timeout)
}
