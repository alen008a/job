...
const (
	MaxIdleConnNum     int = 2000 // 连接池对所有host的最大链接数量，host也即dest-ip, 默认 100
	MaxIdleConnPerHost int = 2000 // 连接池对每个host的最大链接数量
	...
	VideoTimeout       int = 300  // 请求以及连接的超时时间
)
...
func GetVideoProxyClient(timeout time.Duration) *http.Client {
	if config.GetEnv() == config.PRO {
...
func GetShortProxyNotifyClient(timeout time.Duration) *http.Client {
	proxyUrl := config.GetConfig().VerifyProxyUrl
	if proxyUrl == "" {
		proxyUrl = config.GetConfig().ProxyURL
	}
	p, err := url.Parse(proxyUrl)
