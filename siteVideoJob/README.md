...
项目不用重启更改配置,所有配置都是动态配置
数据库初始化不是经常变动的配置，不要动态使用
...
go mod init siteVideoJob go mod tidy
...
bob_BG-100_dev_master

bob_BG-100_merge_testing
```
...
```
go build
siteVideoJob.exe --config=./config/app.dev.ini
```
