...
// 开赛倒计时
func CountDownTime(t string) (string, error) {
	var (
		ret          string
		millis       int64
		timeInMillis int64
	...
	dayOfSeconds = 86400000
	parseTime, err := ParseTime(t)
	if err != nil {
		return "", err
	}
	millis = parseTime.UnixNano() / 1e6
	timeInMillis = time.Now().UnixNano() / 1e6
	if 0 < millis-timeInMillis && millis-timeInMillis < dayOfSeconds {
		ret = fmt.Sprintf("%v", millis-timeInMillis)
	} else if millis-timeInMillis <= 0 {
		ret = "0"
	}
	return ret, err
}

func GetWeek(date string) (s string, err error) {
	parseTime, err := ParseTime(date)
	if err != nil {
		return "", err
	}
	weeks := make(map[string]string, 7)
	weeks["Sunday"] = "周日"
	weeks["Monday"] = "周一"
	weeks["Tuesday"] = "周二"
	weeks["Wednesday"] = "周三"
	...
	weeks["Friday"] = "周五"
	weeks["Saturday"] = "周六"
	cal := parseTime.Weekday().String()
	return weeks[cal], err
}
