...
func InitMysql() (err error) {
	...
	sqlDB.EdgeDB, err = initSqlDB(config.GetConfig().EdgeDB,"Edge")
	if err != nil {
		glog.Emergency("init EdgeDB db is err: %v", err)
		return err
	}
...
func initSqlDB(c config.Mysql, dbName string) (*gorm.DB, error) {
	...
	db, err := gorm.Open(
		mysql.New(
			mysql.Config{
				DSN: address, //apollo有编码问题
				//DSN: strings.Replace(c.Address, `\u0026`, "&", -1), //apollo有编码问题
			},
		), &gorm.Config{
			PrepareStmt:            false,
